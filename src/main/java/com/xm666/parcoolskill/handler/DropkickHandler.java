...
public class DropkickHandler {
    public static final ResourceKey<DamageType> PLAYER_ATTACK_DROPKICK =
            ResourceKey.create(Registries.DAMAGE_TYPE, ResourceLocation.fromNamespaceAndPath(ParCoolSkill.MODID, "dropkick"));
    ...
    static void onLivingIncomingDamage(LivingIncomingDamageEvent event) {
        ...
        //    if (parkourability.get(CatLeap.class).isDoing() && parkourability.get(Slide.class).isDoing()) {
        //        var strength = player.getKnockback(targetEntity, event.getSource());
        //        targetEntity.knockback(strength * 0.5F, Mth.sin(player.getYRot() * ((float) Math.PI / 180F)), -Mth.cos(player.getYRot() * ((float) Math.PI / 180F)));
        //    }
        //}
    }

    public static void handlePayload(final DropkickPayload payload, final IPayloadContext context) {
        ...
        if (target != null && source instanceof Player player) {
            ...
            DamageSource damageSource = new DamageSource(
                    level.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(PLAYER_ATTACK_DROPKICK),
                    player,
                    player,
                    player.position()
            );
